...
function mergeObjects(objects) {
  ...
  for (let i = 0; i < objects.length; i += 1) {
    const currObj = Object.entries(objects[i]);
    for (let j = 0; j < currObj.length; j += 1) {
      const [key, val] = currObj[j];
...
function makeWord(lettersObject) {
  const str = [];
  const totalLetters = Object.values(lettersObject).reduce(
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    ...
    if (bill === 25) change += 25;
      ...
      if (change >= 25 && change >= 75) {
        change -= 50;
        change -= 25;
      } else if (change >= 75) change -= 75;
      else return false;
    }
  }
  return true;
